...
export default function PartyPlanner() {
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col h-[calc(100vh-8rem)]">
      <div className="mb-3">
        <h2 className="text-xl font-semibold text-slate-800">Party planner</h2>
        <p className="text-slate-600 text-sm mt-1">
          Set budget, headcount, veg/non-veg preferences, or ask for multi-restaurant suggestions.
        </p>
      </div>
      <div className="flex-1 overflow-y-auto space-y-3 p-2 border border-slate-200 rounded-lg bg-slate-50/50 min-h-[200px]">
        {messages.length === 0 && (
          <p className="text-slate-500 text-sm">
            Try: &quot;Plan a party for 10 people, budget ₹5000, mostly veg&quot; or &quot;Suggest
            restaurants for a birthday party in Koramangala&quot;
          </p>
        )}
        {messages.map((m, i) => (
      ...
      <div className="flex gap-2 mt-3">
        <input
          type="text"
          aria-label="Party planning message"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && send()}
          placeholder="e.g. Plan for 10 people, ₹3000, veg options"
